...
<!-- UNVERIFIED -->
Action item: verify `concurrencyKey` runtime behavior against SDK 4.4.3 at `https://trigger.dev/docs/queue-concurrency`.
...
<!-- UNVERIFIED -->
Action item: verify 5-second checkpoint threshold behavior at `https://trigger.dev/docs/queue-concurrency`.
...
<!-- UNVERIFIED -->
Action item: verify deadlock prevention mechanism details at `https://trigger.dev/docs/queue-concurrency`.
...
Queue depth is the maximum number of runs that can be enqueued per queue (not per environment):
...
- [Master Reference — Limits](./TRIGGER_DEV_MASTER.md)
...
- [Management API](./TRIGGER_DEV_MANAGEMENT_API.md) — queue management endpoints
